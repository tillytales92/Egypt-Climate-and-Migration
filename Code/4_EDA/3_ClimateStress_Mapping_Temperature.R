...
#Households
hh_datasample <- read_dta(paste(here(),
                           "Data",
                           "hh_gps_datasample.dta",sep = "/"))
...
dailymean_1980 <- mean(temp_era5_dailymean_1980s - 273.15, na.rm = TRUE)
dailymean_2000 <- mean(temp_era5_dailymean_2000s - 273.15, na.rm = TRUE)
dailymean_2020 <- mean(temp_era5_dailymean_2020s - 273.15, na.rm = TRUE)
...
mean_temp_julaug_2000_2004 <- mean(temp_subset - 273.15, na.rm = TRUE)
...
# compute mean across all selected days (°C)
mean_temp_julaug_2020_2024 <- mean(temp_subset_2020s - 273.15, na.rm = TRUE)
...
# convert Kelvin to °C
temp_subset_winter_2000s_c <- temp_subset_winter_2000s - 273.15

# count number of days > 30 °C per cell
days_above30_winter_2000s <- sum(temp_subset_winter_2000s_c > 30, na.rm = TRUE)
...
# subset rasters for those days
temp_subset_winter <- temp_era5_dailymax_19602024[[idx_october_2020s]]

# convert Kelvin to °C
temp_subset_winter_c <- temp_subset_winter - 273.15

# count number of days > 30 °C per cell
days_above30_winter <- sum(temp_subset_winter_c > 30, na.rm = TRUE)

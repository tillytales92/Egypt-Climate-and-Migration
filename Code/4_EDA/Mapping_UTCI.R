#Comparisons of UTCI with Temperature values
#Read in Packages --------------------------------------------------------
#Define packages used
libs <- c("tidyverse","here","lubridate","terra","raster","sf",
          "exactextractr","patchwork","weathermetrics")
...
#Load Data ---------------------------------------------------------------
####Households####
hh_data_imputedgps <- readRDS(file = paste(here(),"Data","intermediate",
                                           "Household Data",
...
era5_temp_df <- readRDS(file = paste(here(),
                                     "Data", "intermediate","Governorate Data",
                                     "era5_temp_19602024",sep = "/"))

####Raster: UTCI####
utci_daily_max_rast <- terra::rast(
                   paste(here(),"Data", "intermediate","UTCI",
...
plot(
  utci_gap_p95,
  main = "Areas where UTCI diverges most from measured air temperature\nP95 of |UTCI − T| (°C)"
)
...
plot(
  utci_gap_mean,
  main = "Where UTCI exceeds measured air temperature (positive) \nvs. falls below (negative)"
)
plot(egypt_gov,add = TRUE,color = "black")
